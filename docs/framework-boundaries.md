...
- `jfoundry-messaging-spring-boot-starter`：聚合 messaging application contracts、Jackson payload serializer、Spring domain-event dispatcher 和默认 logging `MessageSender`。
...
- `jfoundry-mybatis-plus-spring-boot-starter`：只聚合业务聚合根 repository 所需的 MyBatis-Plus persistence adapter，不隐式包含 Outbox/Inbox store。
