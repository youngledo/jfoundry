...
`jfoundry-infrastructure` hosts framework-neutral technical adapters implementing or consuming application-layer contracts.
技术适配器实现核心契约，但不负责 Spring、Helidon、Quarkus 等运行时框架集成，也不负责框架启动和自动装配：
...
这里的 `infrastructure` 表示 Onion simplified 中的基础设施环。它按技术关注点组织模块（MyBatis-Plus、Kafka、RocketMQ、RabbitMQ、JobRunr 等），但不承载应用运行时框架本身。Spring 相关 adapter 统一归入 `jfoundry-spring`，未来 Helidon、Micronaut、Quarkus 等运行时也应以平级集成模块扩展。
