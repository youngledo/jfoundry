...
- `jfoundry-spring/jfoundry-spring-boot-starter-core`
...
starter 是业务项目优先依赖的聚合入口。新项目应显式选择：

- `jfoundry-spring-boot-starter-core`：ORM-neutral，只聚合 Spring Boot 自动装配、DDD/application ports、Jackson/Spring messaging 和 Outbox Spring runtime；业务侧自行提供 Spring Data/JPA/其它持久化 adapter。
- `jfoundry-spring-boot-starter-mybatis-plus`：在 core starter 之上聚合 MyBatis-Plus boot runtime，以及 Persistence / Outbox / Inbox 的 MyBatis-Plus adapter。
- `jfoundry-spring-boot-starter`：兼容旧 artifactId，当前委托到 MyBatis-Plus starter。
...
`jfoundry-messaging-kafka` 只放 Kafka `MessageSender` adapter，不进入 jfoundry Spring Boot starter。业务侧显式选择 Kafka 时再引入该模块，未来切换 MQ 通过替换 `MessageSender` adapter 完成，不影响 Outbox core。

MyBatis-Plus adapter 不进入 `jfoundry-spring-boot-starter-core`。如果业务使用 Spring Data / JPA，应依赖 core starter，并通过业务侧或后续扩展模块提供 `OutboxMessageStore`、`InboxMessageStore` 和 repository 实现。
...
- Spring Boot starter 继续作为业务项目的集成入口，但新项目必须显式选择 core 或 MyBatis-Plus starter。
