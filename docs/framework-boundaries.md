...
`jfoundry-messaging-kafka` 只放 Kafka `MessageSender` adapter，不进入默认 Spring Boot starter。业务侧显式选择 Kafka 时再引入该模块，未来切换 MQ 通过替换 `MessageSender` adapter 完成，不影响 Outbox core。
