...
- `jfoundry-spring-boot-starter`：ORM-neutral 的默认入口，只聚合 Spring Boot 自动装配、DDD/application ports、Jackson/Spring messaging 和 Outbox Spring runtime；业务侧自行提供 Spring Data/JPA/其它持久化 adapter。
- `jfoundry-spring-boot-starter-mybatis-plus`：在默认 starter 之上聚合 MyBatis-Plus boot runtime，以及 Persistence / Outbox / Inbox 的 MyBatis-Plus adapter。
...
MyBatis-Plus adapter 不进入 `jfoundry-spring-boot-starter`。如果业务使用 Spring Data / JPA，应依赖默认 starter，并通过业务侧或后续扩展模块提供 `OutboxMessageStore`、`InboxMessageStore` 和 repository 实现。
...
- Spring Boot starter 继续作为业务项目的集成入口；默认 starter 保持 ORM-neutral，MyBatis-Plus starter 作为可选运行时变体叠加。
